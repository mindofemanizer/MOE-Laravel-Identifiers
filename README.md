...
1. **Public ID obfuscation** â€” sembunyikan integer primary key (`1, 2, 3, ...`) di URL/API publik untuk mencegah IDOR & enumerasi. Trait: `HasPublicId`.
2. **Document numbering** â€” nomor dokumen yang mudah dibaca manusia untuk Order, Invoice, Ticket, dll. Trait: `HasDocumentNumber`.
...
```bash
composer require moe/laravel-identifiers:dev-main
```
...
## Kebutuhan 1 â€” Public ID (`HasPublicId`)
...
```php
$user->public_id;              // "bF3aX9kQ" â€” bukan "1"
...
## Kebutuhan 2 â€” Document Number (`HasDocumentNumber`)
...
| `{RAND:n}` | `n` karakter acak CSPRNG (alfabet non-ambigu) | `{RAND:6}` â†’ `XWK5HB` |
| `{SEQ:n}` | Nomor urut gap-safe, di-pad `n` digit | `{SEQ:5}` â†’ `00001` |

**Alfabet `{RAND}`** default: `23456789ABCDEFGHJKMNPQRSTVWXYZ` (tanpa `0/O`, `1/I/L`, `U`) untuk menghindari salah baca.

**Perbedaan `{RAND}` vs `{SEQ}`:**

- `{RAND}` â€” acak, tidak berurutan (tidak bisa ditebak/enumerasi), unik dicek ke DB (retry sampai 10x).
- `{SEQ}` â€” berurutan & rapi, gap-safe via transaksi + `lockForUpdate`. Reset periodik (default tahunan) diatur di config.
...
> **Penting:** ganti `MOE_ID_SQIDS_ALPHABET` per-project (alfabet berbeda â‡’ output berbeda). Jangan ubah alfabet setelah produksi berjalan karena akan memutus URL lama.
